  ...
  "game/web/src/render/mapRenderer.ts"() {
    ...
    _SvgMapRenderer = class _SvgMapRenderer {
      constructor(svgEl, getState, paintStroke, setActiveDistrict) {
        ...
        __publicField(this, "partyLabels", {});
      ...
      renderCountyBorders() {
        this.countyBorderGroup.selectAll("line.county-boundary").data(this.countySegments).join(
          ...
          (update) => update,
          (exit) => exit.remove()
        ).attr("x1", (d) => d.x1).attr("y1", (d) => d.y1).attr("x2", (d) => d.x2).attr("y2", (d) => d.y2).attr("stroke", "#1c1c1c").attr("stroke-width", _SvgMapRenderer.COUNTY_BASE_WIDTH / this.currentK).attr("stroke-dasharray", `${_SvgMapRenderer.COUNTY_DASH_ON / this.currentK},${_SvgMapRenderer.COUNTY_DASH_OFF / this.currentK}`).attr("opacity", _SvgMapRenderer.COUNTY_OPACITY);
      }
        ...
        svgNode2.addEventListener("mousemove", (event) => {
          ...
          if (this.hoveredPath !== path2) {
            ...
            if (infoPanel !== null) {
              ...
              const partyName = (_b = this.partyLabels[topParty]) != null ? _b : PARTY_LABELS[topParty];
              ...
              const countyHtml = d.county_name ? `<span style="color:#8898b0">${d.county_name}</span><br>` : "";
              infoPanel.innerHTML = `<div class="precinct-name">${precinctLabel}</div><div class="precinct-detail">` + countyHtml + `${distLabel}<br>Pop: ${d.population.toLocaleString()}<br>Lean: ${leanLabel}` + groupsHtml + `</div>`;
            }
          }
        });
...
function runOverlay(id2, script, store) {
  ...
  function clearStepDecorations() {
    ...
    document.querySelectorAll(".tutorial-interactive").forEach(
      (el) => el.classList.remove("tutorial-interactive")
    );
  ...
  function teardown() {
    ...
    overlayAc.abort();
    panel.remove();
    document.querySelectorAll(".tutorial-reveal-hidden").forEach(
      (el) => el.classList.remove("tutorial-reveal-hidden")
    );
...
var PANEL_ID, activeOverlayTeardown, TUTORIAL_001, TUTORIAL_002, TUTORIAL_003, TUTORIAL_004, SCRIPTS;
...
function scenarioToSpike(scenario) {
  ...
  const precincts = scenario.precincts.map((pc, i) => {
    ...
    if (pc.county_name !== void 0)
      spikePrecinct.county_name = pc.county_name;
...
var require_main = __commonJS({
  "game/web/src/main.ts"(exports) {
    ...
    (() => __async(exports, null, function* () {
      ...
      if (!showResults) {
        (_f = document.getElementById("results-heading")) == null ? void 0 : _f.style.setProperty("display", "none");
        resultsEl.style.display = "none";
      }
      if (!showValidity) {
        (_g = document.getElementById("validity-heading")) == null ? void 0 : _g.style.setProperty("display", "none");
      ...
      function applyFiltersCollapsed(collapsed) {
        ...
        mapFiltersToggle == null ? void 0 : mapFiltersToggle.setAttribute("aria-label", collapsed ? "Expand view filters" : "Collapse view filters");
      ...
      function applyDistrictsCollapsed(collapsed, persist = true) {
        ...
        districtToolbarToggle == null ? void 0 : districtToolbarToggle.setAttribute("aria-label", collapsed ? "Expand district painter" : "Collapse district painter");
      ...
      (_h = document.getElementById("btn-wrap-up-replay")) == null ? void 0 : _h.addEventListener("click", () => {
        window.location.assign(backUrl);
      });
      store.subscribe(() => {
        updateUI();
