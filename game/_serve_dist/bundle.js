...
function makePrng(seed) {
  let s2 = seed >>> 0;
  return () => {
    s2 = Math.imul(1664525, s2) + 1013904223;
    return (s2 >>> 0) / 4294967296;
  };
}
function gaussianContribution(dq, dr, epi) {
  const dx = dq;
  const dy = dr + dq * 0.5;
  const d2 = dx * dx + dy * dy;
  return epi.weight * Math.exp(-d2 / (2 * epi.sigma * epi.sigma));
}
function makePartisanField(rng) {
  const waves = [
    {
      angle: rng() * Math.PI,
      freq: 0.3 + rng() * 0.2,
  ...
  return (q, r) => {
    ...
    for (const w of waves) {
      ...
      sum4 += Math.sin(w.freq * proj + w.phase);
    }
    return (sum4 / 3 + 1) / 2;
  };
}
function generatePrecincts(seed = 42) {
  ...
  coords.forEach((c3, i) => idMap.set(`${c3.q},${c3.r}`, i));
  const precincts = coords.map(({ q, r }, id2) => {
    let pop = 0;
    ...
    const partyShare = {
      D: Math.round(dShare * 1e3) / 1e3,
      R: Math.round(rShare * 1e3) / 1e3,
      L: Math.round(lShare * 1e3) / 1e3,
      G: Math.round(gShare * 1e3) / 1e3,
      I: Math.round(iShare * 1e3) / 1e3
    };
    const total = partyShare.D + partyShare.R + partyShare.L + partyShare.G + partyShare.I;
    partyShare.D = Math.round(partyShare.D / total * 1e3) / 1e3;
    partyShare.R = Math.round(partyShare.R / total * 1e3) / 1e3;
    partyShare.L = Math.round(partyShare.L / total * 1e3) / 1e3;
    partyShare.G = Math.round(partyShare.G / total * 1e3) / 1e3;
    partyShare.I = Math.round((1 - partyShare.D - partyShare.R - partyShare.L - partyShare.G) * 1e3) / 1e3;
    const parties = ["D", "R", "L", "G", "I"];
    const winner = parties.reduce((a2, b) => partyShare[a2] > partyShare[b] ? a2 : b);
    const second2 = parties.filter((p) => p !== winner).reduce((a2, b) => partyShare[a2] > partyShare[b] ? a2 : b);
    const margin = Math.round((partyShare[winner] - partyShare[second2]) * 100) / 100;
    const previousResult = { winner, margin };
    const nb = 0.015 + rng() * 0.01;
    const male = (1 - nb) * (0.48 + rng() * 0.04);
    const female = 1 - nb - male;
    const demographics = {
      male: Math.round(male * 1e3) / 1e3,
      female: Math.round(female * 1e3) / 1e3,
      nonbinary: Math.round(nb * 1e3) / 1e3
    };
    const neighbors = HEX_DIRECTIONS.map(([dq, dr]) => {
      const nKey = `${q + dq},${r + dr}`;
      const nId = idMap.get(nKey);
      return nId !== void 0 ? nId : null;
    });
...
var MapRenderer = class {
  constructor(svgEl2, getState, paintStroke, setActiveDistrict) {
    ...
    this.svg = select_default2(svgEl2);
  ...
  render() {
    ...
    this.hexGroup.selectAll("path.hex").data(precincts, (d) => String(d.id)).join(
      (enter) => enter.append("path").attr("class", "hex").attr("d", (d) => hexPolygonPath(d)).attr("stroke", "none").attr("stroke-width", 0.5).style("cursor", "crosshair"),
      (update) => update,
      (exit) => exit.remove()
  ...
  renderBoundaries(segments) {
    this.borderGroup.selectAll("line.boundary").data(segments).join(
      ...
      (update) => update,
      (exit) => exit.remove()
    ).attr("x1", (d) => d.x1).attr("y1", (d) => d.y1).attr("x2", (d) => d.x2).attr("y2", (d) => d.y2).attr("stroke", "#ffffff").attr("stroke-width", 1.5).attr("opacity", 0.6).attr("stroke-dasharray", null);
  }
  ...
  updateBoundaryPreview() {
    ...
    this.previewBorderGroup.selectAll("line.preview-boundary").data(segments).join(
      ...
      (update) => update,
      (exit) => exit.remove()
    ).attr("x1", (d) => d.x1).attr("y1", (d) => d.y1).attr("x2", (d) => d.x2).attr("y2", (d) => d.y2).attr("stroke", "#ffffff").attr("stroke-width", 2).attr("stroke-dasharray", "5,4").attr("opacity", 0.85);
  }
  ...
  initHoverEvents() {
    ...
    svgNode2.addEventListener("mousemove", (event) => {
      ...
      if (this.hoveredPath !== path2) {
        ...
        select_default2(path2).attr("stroke", "#ffffff").attr("stroke-width", 1.5).attr("opacity", 0.95);
...
var temporal = (config, options) => {
  const configWithTemporal = (set3, get3, store2) => {
    var _a, _b;
    store2.temporal = createStore(
    ...
    const curriedHandleSet = ((_b = options == null ? void 0 : options.handleSet) == null ? void 0 : _b.call(
      options,
      store2.temporal.getState()._handleSet
    )) || store2.temporal.getState()._handleSet;
    const temporalHandleSet = (pastState) => {
      var _a2, _b2, _c;
      if (!store2.temporal.getState().isTracking)
        return;
    ...
    const setState = store2.setState;
    store2.setState = (...args) => {
      var _a2;
...
// web/src/simulation/election.ts
var ALL_PARTIES = ["R", "D", "L", "G", "I"];
function zeroShare() {
...
var useGameStore = createStore()(
  temporal(
    (set3, get3) => __spreadProps(__spreadValues({}, buildInitialState()), {
      setActiveDistrict(id2) {
        set3({ activeDistrict: id2 });
      },
      paintPrecinct(precinctId) {
        const { assignments, activeDistrict } = get3();
      ...
      paintStroke(precinctIds, district) {
        ...
        if (!changed)
          return;
        const nextState = __spreadProps(__spreadValues({}, get3()), { assignments: next });
        set3({
          assignments: next,
...
var useTemporalStore = () => useGameStore.temporal;
...
if (svgEl === null || resultsEl === null || legendEl === null || districtBtnsEl === null || btnUndo === null || btnRedo === null || btnViewToggle === null) {
...
function updateUI() {
  ...
  const temporal2 = useTemporalStore();
  const { pastStates, futureStates } = temporal2.getState();
  renderer.render();
  if (resultsEl !== null) {
...
var currentViewMode = "districts";
btnViewToggle.addEventListener("click", () => {
  currentViewMode = currentViewMode === "districts" ? "lean" : "districts";
  renderer.setViewMode(currentViewMode);
  btnViewToggle.textContent = currentViewMode === "districts" ? "View: Partisan Lean" : "View: Districts";
});
store.subscribe(() => updateUI());
useTemporalStore().subscribe(() => updateUI());
updateUI();
var wasmEl = document.getElementById("wasm-status");
if (wasmEl !== null) {
