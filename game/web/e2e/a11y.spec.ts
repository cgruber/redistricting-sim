...
test.describe("GAME-105 focus management & inert", () => {
	 ...
	 */
	test("result overlay makes the editor inert (cannot reach the map behind it)", async ({
		page,
	}) => {
		await loadEditor(page);
			...
			page.evaluate(() => {
				const store = (
					window as unknown as Record<
						string,
						{ getState: () => { assignments: Map<number, number | null> } }
					>
				)["__gameStore"];
	...
	test("scenario cards have role=listitem", async ({ page }) => {
		...
		await expect(page.locator("#scenario-cards .scenario-card").first()).toBeVisible({
			timeout: 10_000,
		});
	 ...
	 */
	test("nav-back submenu: focus first item on open, arrow roving, Escape returns to trigger", async ({
		page,
	}) => {
...
test.describe("GAME-055 Party names", () => {
	 ...
	 */
	test("scenario-002 results panel shows scenario party names not hardcoded labels", async ({
		page,
	}) => {
		...
		await expect(page.locator("#results-container .result-district").first()).toBeVisible({
			timeout: 5_000,
		});
	 ...
	 */
	test("scenario-009 results panel shows scenario party names not hardcoded labels", async ({
		page,
	}) => {
