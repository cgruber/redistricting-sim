...
test("filter toolbar: present; coloring radio + overlay sections, defaults correct", async ({
	page,
}) => {
	await loadApp(page);
...
test("reset: full flow — paint, confirm reset, fills restored, undo disabled", async ({ page }) => {
	await loadApp(page);
	const hex0 = page.locator("path.hex[data-precinct-id='0']");
	const btnUndo = page.locator("#btn-undo");
