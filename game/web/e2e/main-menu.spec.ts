...
test("main menu: Continue visible but disabled when no campaign progress exists", async ({
	page,
}) => {
	await page.goto("/");
...
test("main menu: Continue enabled when lastPlayedScenarioId set in localStorage", async ({
	page,
}) => {
	await page.goto("/");
	await page.evaluate(
