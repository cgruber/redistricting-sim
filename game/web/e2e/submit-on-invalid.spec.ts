...
test("submit-on-invalid: submit button is enabled on initial load (no validity gate)", async ({
	page,
}) => {
...
test("submit-on-invalid: Fix It button returns to editor from invalid map result", async ({
	page,
}) => {
	await loadEditor(page);
	await paintAllIntoDistrictOne(page);
...
test("submit-on-invalid: any non-passing result hides Next Scenario and shows the back button", async ({
	page,
}) => {
...
test("submit-on-invalid: passing map shows pass screen routing to the next scenario", async ({
	page,
}) => {
	await loadEditor(page);
