 ...
 */
async function paintPrecinct(page: import("@playwright/test").Page, idx: number): Promise<void> {
...
async function selectDistrict(page: import("@playwright/test").Page, nth: number): Promise<void> {
...
test("scenario-002: governor sprite appears in the sc-ken-seats criterion row", async ({
	page,
}) => {
	...
	await expect(
		page.locator(".result-criterion .rc-char .character-sprite.character-governor").first(),
	).toBeVisible();
});

test("scenario-002 winnability: packing east Ryu bloc into one district passes", async ({
	page,
}) => {
	/**
...
test("scenario-004 winnability: cracking the corridor across all 5 districts passes", async ({
	page,
}) => {
	/**
	 * Hex-of-hexes R=6: 127 precincts, 5 districts of ~25.
...
test("scenario-005 winnability: consolidating the valley into one district passes", async ({
	page,
}) => {
	/**
	 * Hex-of-hexes R=6: 127 precincts, 5 districts of ~25.
...
test("scenario-006 winnability: column strips separate partisan flanks into safe seats", async ({
	page,
}) => {
	/**
	 * Hex-of-hexes R=6: 127 precincts, 5 districts of ~25.
...
test("scenario-007 winnability: five compact blobs pass reform criteria", async ({ page }) => {
	...
	await page.evaluate(() => {
		...
		const assignment: number[][] = [
			[
				33, 42, 43, 44, 52, 53, 54, 55, 56, 63, 64, 65, 66, 67, 68, 69, 77, 78, 79, 80, 81, 90, 91,
				92, 101, 102,
			],
			[
			...
			[
				59, 60, 61, 62, 70, 71, 72, 73, 74, 75, 82, 83, 84, 85, 86, 93, 94, 95, 96, 103, 104, 105,
				112, 113, 120,
			],
			[
				0, 1, 7, 8, 9, 15, 16, 17, 24, 25, 26, 27, 34, 35, 36, 37, 38, 45, 46, 47, 48, 49, 50, 57,
				58,
			],
			[
				2, 3, 4, 5, 6, 10, 11, 12, 13, 14, 18, 19, 20, 21, 22, 23, 28, 29, 30, 31, 32, 39, 40, 41,
				51,
			],
		];
...
test("scenario-009 winnability: ring-based split gives 3 Cat-safe districts", async ({ page }) => {
	...
	await page.evaluate(() => {
		...
		const assignment: number[][] = [
			[
				27, 28, 29, 30, 37, 38, 39, 40, 41, 49, 50, 51, 52, 61, 62, 63, 64, 65, 74, 75, 76, 77, 86,
				87, 88,
			],
			[
				55, 66, 67, 68, 72, 73, 78, 79, 80, 83, 84, 85, 89, 90, 95, 96, 97, 98, 99, 100, 105, 106,
				107, 108, 109,
			],
			[
				8, 9, 10, 11, 12, 13, 16, 17, 18, 19, 20, 21, 22, 25, 26, 31, 32, 36, 42, 47, 48, 53, 54,
				59, 60,
			],
			[
				82, 91, 92, 93, 94, 101, 102, 103, 104, 110, 111, 112, 113, 114, 115, 116, 117, 118, 119,
				120, 121, 122, 123, 124, 125, 126,
			],
			[
				0, 1, 2, 3, 4, 5, 6, 7, 14, 15, 23, 24, 33, 34, 35, 43, 44, 45, 46, 56, 57, 58, 69, 70, 71,
				81,
			],
		];
		assignment.forEach((ids, d) => paintStroke(ids, d + 1));
...
test("scenario select: all cards visible and scrollable", async ({ page }) => {
	...
	await page.evaluate(() => {
		localStorage.setItem(
			...
			JSON.stringify({ completed: ["tutorial-002"] }),
		);
	});
	await page.reload();
...
test("debug force-win button: visible with ?debug param, marks scenario complete", async ({
	page,
}) => {
...
test("reset: also restarts the guided tutorial in place (zeroes the whole scenario)", async ({
	page,
}) => {
...
test("reset: restores the initial view — districts colouring + county overlay off", async ({
	page,
}) => {
...
test("wrap-up screen: completing last scenario shows wrap-up on Next Scenario", async ({
	page,
}) => {
...
test("tutorial-001 strips chrome to paint-only (no prediction, validity, view toolbar, or legend)", async ({
	page,
}) => {
...
test("tutorial-001 winnability: carving a balanced contiguous District 2 passes", async ({
	page,
}) => {
	/**
...
test("tutorial-001: a wildly imbalanced split still passes (balance not enforced)", async ({
	page,
}) => {
...
test("guided overlay: walks the player from orient → pick D2 → paint → undo → submit", async ({
	page,
}) => {
...
test("guided overlay: tutorial-002 runs the legal-map script (orient → paint → validity → Done)", async ({
	page,
}) => {
	// resetTutorial=1 clears the suppression flag the beforeEach set, so the overlay runs.
...
test("in-game nav: back trigger is shown in header when playing a scenario via campaign", async ({
	page,
}) => {
...
test("in-game nav: clicking trigger reveals both Return to Scenarios and Return to Main Menu", async ({
	page,
}) => {
	await page.goto("/?campaign=tutorial&s=tutorial-001");
	const skip = page.locator("#btn-intro-skip");
...
test("in-game nav: without campaign context, shows plain Main Menu button (no dropdown)", async ({
	page,
}) => {
	await page.goto("/?s=tutorial-002");
	const skip = page.locator("#btn-intro-skip");
...
test("GAME-066: reduced-motion — all criterion rows visible immediately after submit", async ({
	page,
}) => {
...
test("tutorial-003 chrome: the result panel AND the Map Validity panel are both visible", async ({
	page,
}) => {
...
test("tutorial-003 lean view: switching to Lean recolors the map by partisanship", async ({
	page,
}) => {
	await loadScenario(page, "tutorial-003");
...
test("tutorial-003 winnability: three balanced, connected wedges pass (district_count + balance + contiguity)", async ({
	page,
}) => {
	await loadScenario(page, "tutorial-003");
	...
	await page.evaluate(() => {
		...
		state.precincts.forEach((p: { coord: { q: number; r: number } }, i: number) => {
			...
			const wedge = Math.min(
				2,
				Math.floor(((a - off + 2 * Math.PI) % (2 * Math.PI)) / ((2 * Math.PI) / 3)),
			);
			if (wedge === 1) d2.push(i);
			else if (wedge === 2) d3.push(i);
			// wedge 0 stays District 1
...
test("guided overlay: tutorial-003 reveals the result + lean, then county, then a Done beat", async ({
	page,
}) => {
	// resetTutorial=1 clears the suppression flag the beforeEach set, so the overlay runs.
...
test("guided overlay: tutorial-004 gives a light orient then a read-only Done beat that unlocks without submitting", async ({
	page,
}) => {
	...
	await expect(page.locator("#tutorial-panel")).toContainText("the map is yours", {
		timeout: 10_000,
	});
	await expect(page.locator("#map-svg")).not.toHaveClass(/tutorial-interactive/);
...
test("scenario-002 verdict DOM: a winning submission shows filled stars and the pass verdict", async ({
	page,
}) => {
