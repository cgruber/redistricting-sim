...
test("GAME-052: criterion rows all visible immediately in reduced-motion mode", async ({
	page,
}) => {
...
test("GAME-069: reduced-motion path sets all rc-char-verdict elements to opacity 1", async ({
	page,
}) => {
	await loadEditor(page);
	await openResultScreen(page);
...
test("GAME-052: final state has correct criteria count after skip (regression)", async ({
	page,
}) => {
	await loadEditor(page);
	await openResultScreen(page);
...
test("GAME-073: success banner and stars shown immediately in reduced-motion mode (force-win)", async ({
	page,
}) => {
	await page.goto("/?s=scenario-002&debug");
	await page.emulateMedia({ reducedMotion: "reduce" });
