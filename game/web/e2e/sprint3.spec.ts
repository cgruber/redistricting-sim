...
test("intro: first slide heading is shown and Previous is disabled", async ({ page }) => {
	await page.goto("/?s=tutorial-002");
	await expect(page.locator("#intro-screen")).toBeVisible({ timeout: 10_000 });
	await expect(page.locator("#intro-slide-heading")).toHaveText(
		"Back to Millbrook — Now With Rules",
	);
...
test("submit: button is enabled on initial load (GAME-059: validity gate removed)", async ({
	page,
}) => {
...
test("submit: button remains in DOM and is interactive after painting precincts across districts", async ({
	page,
}) => {
	await loadEditor(page);
...
test("progression: scenario card shows Completed status for completed scenario", async ({
	page,
}) => {
	await seedProgress(page, ["tutorial-002"]);
	await page.goto("/?campaign=tutorial");
...
test("wip: painting precincts triggers a debounced WIP save to localStorage", async ({ page }) => {
		...
		const store = (window as unknown as Record<string, unknown>)["__gameStore"] as
			| {
					getState: () => {
						...
						setActiveDistrict: (d: number) => void;
					};
			  }
			| undefined;
	...
	const wip = JSON.parse(wipRaw!) as {
		scenarioId: string;
		assignments: Record<string, number>;
		activeDistrict: number;
	};
...
test("wip: WIP is cleared from localStorage after scenario completion", async ({ page }) => {
	...
	await page.evaluate(() => {
		...
		state.precincts.forEach((p: { coord: { q: number; r: number } }, i: number) => {
			...
			if (p.coord.q < 0)
				southWest.push(i); // rural south-west → District 2
...
test("winnability: drawing balanced, contiguous districts produces a passing map", async ({
	page,
}) => {
	/**
...
test("legal-map chrome: validity panel shows; view toolbar + election prediction stay hidden", async ({
	page,
}) => {
...
test("winnability (negative): a lopsided three-district map is flagged by the panel and fails", async ({
	page,
}) => {
	...
	await expect(
		page.locator("#validity-container .validity-row.validity-error").first(),
	).toBeVisible();
