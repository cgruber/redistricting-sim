...
test("populateScenario: all precincts receive total_population", () => {
	...
	assertEqual(
		result.precincts.every((p) => p.total_population !== undefined),
		true,
	);
});
...
test("populateScenario: overwrites existing total_population", () => {
	...
	assertEqual(
		result.precincts.every((p) => p.total_population !== 99999),
		true,
	);
});
...
test("settlement: east anchor places bump at max-q precinct", () => {
	...
	const atMaxQ = result.precincts.filter(
		(p) => (p.position as { q: number; r: number }).q === maxQ,
	);
	const others = result.precincts.filter(
		(p) => (p.position as { q: number; r: number }).q !== maxQ,
	);
...
test("resolveAnchor: HexPos anchor throws when no precinct at position", () => {
	...
	let threw = false;
	try {
		populateScenario(makePartial(1), spec);
	} catch {
		threw = true;
	}
...
test("resolveAnchor: feature anchor throws when no matching precincts (no sea tiles)", () => {
	...
	let threw = false;
	try {
		populateScenario(makeTerrainPartial(), spec);
	} catch {
		threw = true;
	}
	assertEqual(threw, true);
...
test("gradient: center precinct outweighs the rim when strength > 0", () => {
	...
	const maxOther = Math.max(
		...result.precincts.filter((p) => p.id !== center.id).map((p) => p.total_population!),
	);
	assertEqual(center.total_population! > maxOther, true);
});
...
test("smoothing: reduces the spread of an i.i.d. jitter field", () => {
	...
	const smooth = populateScenario(
		partial,
		basePopSpec({ base: 1000, variance: 500, smoothing: 3 }),
	);
