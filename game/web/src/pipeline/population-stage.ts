...
import type { HexPos, PopulationSpec, SettlementAnchor, TerrainWeightsSpec } from "./spec-types.js";
...
export function populateScenario(partial: PartialScenario, spec: PopulationSpec): PartialScenario {
	...
	for (const settlement of spec.settlements ?? []) {
		const anchorPos = resolveAnchor(settlement.anchor, partial, suitabilities, terrainContexts);
