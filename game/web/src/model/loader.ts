...
type RawPrecinct = Omit<Precinct, "initial_district_id"> & {
...
function parsePrecinct(raw: unknown, idx: number): PartialPrecinct {
	...
	if (r["initial_district_id"] !== undefined) {
		if (r["initial_district_id"] === null) {
			...
			pc.initial_district_id = requireString(
				r["initial_district_id"],
				`${label}.initial_district_id`,
			) as DistrictId;
		}
	}
...
function parseGroupFilter(raw: unknown, label: string): GroupFilter {
	...
	if ("group_ids" in r) {
		...
		return {
			group_ids: arr.map((x, i) => requireString(x, `${label}.group_ids[${i}]`) as GroupId),
		};
	}
...
function parseEvent(raw: unknown, idx: number): DemographicEvent {
	...
	if (type === "population_shift") {
		...
		if ("precinct_ids" in pfRaw) {
			...
			precinct_filter = {
				precinct_ids: arr.map(
					(x, i) => requireString(x, `${label}.precinct_filter.precinct_ids[${i}]`) as PrecinctId,
				),
			};
		} else if ("tags" in pfRaw) {
			const arr = requireArray(pfRaw["tags"], `${label}.precinct_filter.tags`);
			precinct_filter = {
				tags: arr.map((x, i) => requireString(x, `${label}.precinct_filter.tags[${i}]`)),
			};
...
function parseCriterion(raw: unknown, idx: number): SuccessCriterion {
	...
	switch (cType) {
		case "seat_count":
			criterion = {
				...
				operator: requireString(cRaw["operator"], `${label}.criterion.operator`) as CompareOp,
		...
		case "majority_minority":
			criterion = {
				...
				min_eligible_share: requireNumber(
					cRaw["min_eligible_share"],
					`${label}.criterion.min_eligible_share`,
				),
	...
	if (r["character"] !== undefined) {
		...
		if (!validChars.includes(charVal)) {
			throw new Error(
				`${label}.character: unknown value "${charVal}"; expected one of: ${validChars.join(", ")}`,
			);
		}
...
function parseNarrative(raw: unknown): Narrative {
	...
	if (r["instigator"] !== undefined) {
		throw new Error(
			"narrative.instigator removed; use instigator_character at the scenario root level instead",
		);
	}
	return narrative;
}
...
function parseRules(raw: unknown): ScenarioRules {
	...
	if (
		contiguityRaw !== "required" &&
		contiguityRaw !== "preferred" &&
		contiguityRaw !== "allowed"
	) {
	...
	if (r["compactness_threshold"] !== undefined) {
		rules.compactness_threshold = requireNumber(
			r["compactness_threshold"],
			"rules.compactness_threshold",
		);
	}
	return rules;
}
...
function parseStateContext(raw: unknown): StateContext {
	...
	const other_region_results: StateContext["other_region_results"] =
		{} as StateContext["other_region_results"];
	for (const [regionId, rr] of Object.entries(othersRaw)) {
		...
		const seatTotalsRaw = requireObject(
			rrObj["seat_totals"],
			`state_context.other_region_results[${regionId}].seat_totals`,
		);
		const seat_totals: Record<PartyId, number> = {} as Record<PartyId, number>;
		for (const [pid, cnt] of Object.entries(seatTotalsRaw)) {
			seat_totals[pid as PartyId] = requireNumber(
				cnt,
				`state_context.other_region_results[${regionId}].seat_totals[${pid}]`,
			);
		}
		other_region_results[regionId as import("./scenario.js").RegionId] = {
			district_count: requireNumber(
				rrObj["district_count"],
				`state_context.other_region_results[${regionId}].district_count`,
			),
			seat_totals,
		};
	}
...
function parseTerrainTile(raw: unknown, idx: number): TerrainTile {
	...
	if (type !== "sea" && type !== "lake" && type !== "mountain") {
		throw new Error(
			`${label}.type: unknown value "${type}"; expected "sea", "lake", or "mountain"`,
		);
	}
 ...
 */
function validateDimensionFilter(
	gf: GroupFilter,
	schema: GroupSchema | undefined,
	label: string,
): void {
...
function checkUniqueIds(args: {
	...
	if (args.success_criteria !== undefined)
		for (const cr of args.success_criteria) checkId(cr.id, "success_criteria");
}
	...
	for (const pc of precincts) {
		...
		for (const expectedCombo of expectedCombos) {
			...
			if (matchingGroups.length === 0) {
				const comboStr = Object.entries(expectedCombo)
					.map(([k, v]) => `${k}=${v}`)
					.join(", ");
				throw new Error(
...
function parseAllFields(raw: Record<string, unknown>): PartialScenario {
	...
	if (raw["default_district_id"] !== undefined) {
		default_district_id = requireString(
			raw["default_district_id"],
			"default_district_id",
		) as DistrictId;
	}
	...
	if (raw["success_criteria"] !== undefined) {
		success_criteria = requireArray(raw["success_criteria"], "success_criteria").map((c, i) =>
			parseCriterion(c, i),
		);
	}
	...
	if (raw["instigator_character"] !== undefined) {
		...
		if (!validChars.includes(icVal)) {
			throw new Error(
				`instigator_character: unknown value "${icVal}"; expected one of: ${validChars.join(", ")}`,
			);
		}
	...
	if (raw["character_demographics"] !== undefined) {
		...
		for (const key of Object.keys(cd)) {
			if (key === "party") {
				throw new Error(
					`character_demographics: "party" has no demographic variants and must be omitted`,
				);
			}
			if (!validChars.includes(key)) {
				throw new Error(
					`character_demographics: unknown key "${key}"; expected one of: ${validChars.join(", ")}`,
				);
			}
			const val = requireString(
				(cd as Record<string, unknown>)[key],
				`character_demographics.${key}`,
			);
			if (requiresSuffix.has(key) && val === "") {
				throw new Error(
					`character_demographics.${key}: demographic suffix must be non-empty (no bare "${key}/" directory exists)`,
				);
			}
	...
	if (raw["river_edges"] !== undefined) {
		...
		river_edges = edgesRaw.map((pair, i) => {
			...
			if (pairArr.length !== 2) {
				throw new Error(
					`river_edges[${i}]: expected array of 2 precinct IDs, got ${pairArr.length}`,
				);
			}
			return [
	...
	if (raw["river_blocks_contiguity"] !== undefined) {
		river_blocks_contiguity = requireBoolean(
			raw["river_blocks_contiguity"],
			"river_blocks_contiguity",
		);
	}
	...
	if (river_blocks_contiguity !== undefined)
...
function assembleScenario(
	...
	if (partial.default_district_id !== undefined)
		scenario.default_district_id = partial.default_district_id;
	if (partial.instigator_character !== undefined)
		scenario.instigator_character = partial.instigator_character;
	if (partial.character_demographics !== undefined)
	...
	if (partial.river_blocks_contiguity !== undefined)
		scenario.river_blocks_contiguity = partial.river_blocks_contiguity;
	if (partial.hide_election_results !== undefined)
		scenario.hide_election_results = partial.hide_election_results;
	if (partial.hide_view_toolbar !== undefined)
...
export function validateScenarioComplete(partial: PartialScenario): Scenario {
	...
	for (const pc of partial.precincts) {
		if (pc.total_population === undefined) {
			throw new Error(
				`completeness: precinct "${pc.id}" is missing total_population (required for gameplay)`,
			);
		}
		if (pc.demographic_groups === undefined) {
			throw new Error(
				`completeness: precinct "${pc.id}" is missing demographic_groups (required for gameplay)`,
			);
		}
	}
