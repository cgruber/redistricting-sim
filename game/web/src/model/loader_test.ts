...
test("Invariant 12: rejects empty precincts array", () => {
	assertThrows(() => loadScenario(minimalScenario({ precincts: [] })), /Invariant 12/);
});
...
test("auto-fill: absent initial_district_id on editable precinct → districts[0]", () => {
	const result = loadScenario(minimalScenario());
	assertEqual(
		result.precincts[0]?.initial_district_id,
		"d1" as (typeof result.precincts)[0]["initial_district_id"],
	);
});
...
test("auto-fill: absent initial_district_id → default_district_id when set", () => {
	...
	assertEqual(
		result.precincts[0]?.initial_district_id,
		"d2" as (typeof result.precincts)[0]["initial_district_id"],
	);
});
	...
	assertDoesNotThrow(() =>
		loadScenario(
			minimalScenario({
				...
				terrain_tiles: [{ position: { q: 2, r: 0 }, type: "sea" }],
				river_edges: [["p1", "p2"]],
