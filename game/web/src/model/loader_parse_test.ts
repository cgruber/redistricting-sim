...
function fullCustomScenario(overrides: Record<string, unknown> = {}): unknown {
  return fullScenario({
    ...
    precincts: [
      {
        id: "p1", editable: true, position: { x: 0, y: 0 }, neighbors: ["p2"],
        total_population: 1000,
        demographic_groups: [
      ...
      {
        id: "p2", editable: true, position: { x: 1, y: 0 }, neighbors: ["p1"],
        total_population: 1000,
        demographic_groups: [
          { id: "g2", population_share: 1.0, vote_shares: { blue: 0.5, red: 0.5 }, turnout_rate: 0.7 },
        ],
      },
    ],
    ...overrides,
...
test("agreement: duplicate id rejected by both paths (Invariant 11)", () => {
  const json = fullHexScenario({
    districts: [{ id: "d1", name: "District 1" }, { id: "d1", name: "District 1 dupe" }],
  });
...
test("agreement: asymmetric neighbor rejected by both paths (Invariant 8)", () => {
  ...
  const json = fullCustomScenario({
    precincts: [
      ...
      {
        id: "p2", editable: true, position: { x: 1, y: 0 }, neighbors: [],
        total_population: 1000,
        demographic_groups: [
          { id: "g2", population_share: 1.0, vote_shares: { blue: 0.5, red: 0.5 }, turnout_rate: 0.7 },
        ],
...
test("agreement: mountain enclosure rejected by both paths (terrain)", () => {
  ...
  const json = fullHexScenario({
    precincts: [
      ...
      {
        id: "p2", editable: true, position: { q: 5, r: 0 },
        total_population: 1000,
        demographic_groups: [
          { id: "g2", population_share: 1.0, vote_shares: { blue: 0.5, red: 0.5 }, turnout_rate: 0.7 },
        ],
  ...
  partial.terrain_tiles = enclosingMountains.map(m => ({ position: m.position, type: "mountain" as const }));
...
test("agreement: per-group Inv1/Inv6 interleaving — earlier Inv6 wins (parse + complete)", () => {
    ...
    precincts: [
      {
        ...
        total_population: 1000,
        demographic_groups: [
          { id: "g1", population_share: 1.0, vote_shares: { blue: 0.5, red: 0.4 }, turnout_rate: 0.7 },
        ],
      },
      {
        ...
        total_population: 1000,
        demographic_groups: [
          { id: "g2", population_share: 1.0, vote_shares: { blue: 0.5, red: 0.3, green: 0.2 }, turnout_rate: 0.7 },
        ],
      },
    ],
  });
  ...
  partial.precincts[0]!.demographic_groups = [
    { id: "g1" as never, population_share: 1.0, vote_shares: { blue: 0.5, red: 0.4 } as never, turnout_rate: 0.7 },
  ];
  partial.precincts[1]!.demographic_groups = [
    { id: "g2" as never, population_share: 1.0, vote_shares: { blue: 0.5, red: 0.3, green: 0.2 } as never, turnout_rate: 0.7 },
  ];
  ...
  const violatingPrecincts = [
    {
      ...
      total_population: 1000,
      demographic_groups: [
        { id: "g1", population_share: 1.0, vote_shares: { blue: 0.6, red: 0.4 }, turnout_rate: 0.7, dimensions: { race: "white" } },
      ],
    },
    {
      id: "p2", editable: true, position: { q: 1, r: 0 },
      total_population: 1000,
      demographic_groups: [
        { id: "g2", population_share: 1.0, vote_shares: { blue: 0.5, red: 0.5 }, turnout_rate: 0.7, dimensions: { race: "purple" } },
      ],
    },
  ];
