...
| `GAME-102-loader-validation-dedup.md` | game, code-quality, architecture | De-duplicate loader's two scenario-validation passes (`validateStructural` vs `validateScenarioInvariants`, near-identical terrain/river blocks) that can silently drift; decide predicate-extraction vs normalize-then-validate-once (grok-preferred) |
