...
| `GAME-101-silent-failure-hardening.md` | game, code-quality, robustness | Fail loud on silent failure paths: test-harness `process.on('exit')` backstop; `requireNumber` finiteness/range; `evaluateCriteria` exhaustiveness + `assertNever`; WASM `.catch()`/removal. Fix-now batch from the 2026-06-27 quality review |
