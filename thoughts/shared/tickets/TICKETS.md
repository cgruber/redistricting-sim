...
| `GAME-112-multiparty-and-two-party-metrics.md` | game, simulation, architecture | Adapter drops parties past the first two (spike-era R/D positional map); engine + type already support 5. Render third parties AND two-party-normalize efficiency-gap/mean-median (required once minor-party share is nonzero). Coordinate w/ GAME-043. From the 2026-06-30 metrics audit |
...
| `GAME-114-rename-total-population-to-voters.md` | game, simulation, code-quality | Deferred: rename `total_population` → a voter-oriented term (voters/electorate) across schema/loader/types/adapter/pipeline/scenarios so the "these are voters, not census pop" model is self-documenting. Mechanical, no behavior change. Do after GAME-113 |
| `GAME-115-debug-only-campaign.md` | game, UX, tooling | Debug-only campaign (`debugOnly` filtered out of campaign-select unless debug active) to house test/demo scenarios (e.g. the 3-party tutorial-005) without shipping them to normal players. Debug propagation already works (sessionStorage). Supports the GAME-112 multiparty demo |
---
